...
ActivatePAP(mapname)
{
    ...
    switch(mapname)
    {
        case "zm_towers": 
            ...
            wait .5;
            sentinel_model = getent("mdl_pap_quest_sentinel_artifact", "targetname");
            sentinel_model delete();
            wait .5;
            pap_debris_clip = getentarray("mdl_pap_room_debris_clip","targetname");
            foreach (clip in pap_debris_clip)
            {
                clip connectPaths();
                clip delete();
            }
            ...
            foreach(clip2 in var_8a3d82fc)
                clip2 delete();
            exploder::exploder("exp_lgt_pap");
            level flag::set("zm_towers_pap_quest_sentinel_artifact_exploded");
            wait .5;
            var_3075677 = getentarray("mdl_pap_quest_head", "targetname");
            foreach(enemyHead in var_3075677)
            {
                enemyHead flag::clear(#"hash_26125a3306681e2");
                enemyHead delete();
            }
            ...
            var_f7afe1a0 delete();
            self iPrintLnBold("Pap Opened, Walls will still appear");
            break;
        case "zm_escape":
            level flag::set("power_on1");
            level notify(#"hash_7e1d78666f0be68b");
            wait 1;
